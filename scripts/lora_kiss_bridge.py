...
# ---------------------------------------------------------------------------
# Colored logging — Dire Wolf color scheme, black background
# ---------------------------------------------------------------------------
...
class _ColorFormatter(logging.Formatter):
    """
    Applies Dire Wolf-compatible ANSI colors to log output on a black background.
    Used for status/error messages (connection info, warnings, errors, debug).
    Packet RX/TX lines are printed via _dw_print() instead, without a log prefix.
    Colors are suppressed when stderr is not a TTY (e.g. log files, systemd).
    """
...
def _dw_print(heard, packet, color=None):
    """
    Print a packet in Dire Wolf style — no timestamp prefix, just like dw_printf():

      K6ATV-1  RSSI=-95 dBm  SNR=5.0 dB
      [LoRa] K6ATV-1>APLETK,WIDE1-1*:!3707.01N/12134.58W>LoRa APRS

    Long packet lines are wrapped at the terminal width with the '[LoRa] '
    continuation indent so they stay readable on a small screen.
    """
        ...
        while self._running:
            ...
            if got:
                ...
                if n > 0:
                    ...
                    snr     = self._lora.snr()
                    log.debug("RX %d bytes  RSSI=%d dBm  SNR=%.1f dB", n, rssi, snr)
                    try:
...
class LoRaBridge:
    ...
    def _valid_tnc2_header(text):
        ...
        The address field (everything before the first ':') must contain only
        uppercase letters, digits, '>', ',', '-', and '*'.  Any lowercase
        letter or unexpected punctuation indicates a corrupted decode.
        Also validates the source callsign structure.
        """
        colon = text.find(':')
        if colon < 3:
            return False
    ...
    def _on_rx_from_lora(self, payload_bytes, rssi, snr):
        ...
        # Strip leading non-printable preamble bytes (e.g. 0x3C 0xFF 0x01).
        # Only A-Z and 0-9 are valid starts — lowercase signals a corrupt decode.
        tnc2 = re.sub(r'^[^A-Z0-9]+', '', tnc2)
        if not tnc2:
            return

        if not self._valid_tnc2_header(tnc2):
            safe = ''.join(c if c.isprintable() else '?' for c in tnc2)
            log.warning("Dropping packet with invalid TNC2 header (corrupted decode): %s", safe)
            return

        src = tnc2.split('>')[0] if '>' in tnc2 else tnc2[:9]
        _dw_print(f"{src}  RSSI={rssi} dBm  SNR={snr:.1f} dB", tnc2, _ANSI_GREEN)
    ...
    def _on_tx_from_direwolf(self, tnc2_bytes):
        ...
        try:
            ...
            return
        src = tnc2.split('>')[0] if '>' in tnc2 else tnc2[:9]
        _dw_print(src, tnc2, _ANSI_MAGENTA)
        # LoRa APRS standard preamble: 0x3C 0xFF 0x01 ('<' + 0xFF + 0x01)
        # Required by most LoRa APRS firmware (ESP32 iGates, trackers, etc.)
        payload = b'<\xff\x01' + tnc2.encode('ascii', errors='replace')
